...
```c
// row[i][j]: 第i行是否出现数字j
// columns[i][j]: 第i列是否出现数字j
// grid[i][j][k]: 第[i/3, j/3]九宫格中是否出现数字k
bool rows[9][10] = { 0 };
bool columns[9][10] = { 0 };
bool grid[3][3][10] = { 0 };
    ...
    for (int i = 0; i < 9; ++i)
    {
        for (int j = 0; j < 9; ++j)
        {
            if (board[i][j] == '.')
            {
                ...
                {
                    if (!rows[i][num] && !columns[j][num] && !grid[i / 3][j / 3][num]) 
                    {
                        board[i][j] = num + '0';
                        ...
                        if (dfs(board, boardSize, boardColSize)) return true;  // 重点理解
